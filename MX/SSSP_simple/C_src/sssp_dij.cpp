...
int main(int argc, char** argv) {
  ...
  totalDist2 = new unsigned[numVertices];
  ...
  while(!work.empty()) {
	...
	if(!relaxed[it->first]) {
	  ...
	  if(dist < totalDist[it->first]) {
	    ...
	    totalDist2[it->first] = totalDist2[vertex] + 1;
  ...
  cout << "Total nodes executed: " << numNodesExecuted << "\n";

  if(genOutput) {
    for(int i = 0; i < numVertices; i++) {
      out <<  i << "," << totalDist[i] << "," << totalDist2[i] << endl;
    }
  }
}
